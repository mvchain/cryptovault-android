...
interface IGoogleContract {
    abstract class GooglePresenter : BasePresenter<GoogleModel, GoogleView>() {
       abstract fun changeGoogleVerification(googleCode:Int,password:String,status:Int)
    }

    interface GoogleModel : IBaseModel {
        fun changeGoogleVerification(googleCode:Int,password:String,status:Int) :Observable<LoginBean>
    }
